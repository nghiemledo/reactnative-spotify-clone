...
import { deletePodcastEpisodeData } from "@/store/podcast-episode/podcast-episode.actions";
...
export const columns: ColumnDef<PodcastEpisode>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const podCate = podcastShowData.find((g) => g.id === row.original.showId);
            return <>{podCate?.title || "Unknown Show"}</>;
        },
    },
    {
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const handleView = () => {
                navigate(`/admin/podcast-episodes/${row.original.id}`);
            };

            const handleEdit = () => {
                navigate(`/admin/podcast-episodes/${row.original.id}/edit`);
            }

            const handleDelete = async () => {
                setLoading(true);
                try {
                    await dispatch(deletePodcastEpisodeData(row.original.id!)).unwrap();
                    toast('Success', {
